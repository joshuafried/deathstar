...
~/junction/build/junction/junction_run $tmp/post-storage-service.config --ld_path $LD_LIBRARY_PATH -s -- /users/friedj/install/PostStorageService > $tmp/post-storage-service.log 2>&1 &
...
~/junction/build/junction/junction_run $tmp/compose-post-service.config --ld_path $LD_LIBRARY_PATH -s -- /users/friedj/install/ComposePostService > $tmp/compose-post-service.log 2>&1 &
